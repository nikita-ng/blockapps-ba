...
describe('Supply Chain Demo App - deploy contracts', function () {
  ...
  assert.isDefined(config.dataProjectFilename, 'Data argument missing. Set in config, or use --data <path>');
  ...
  it('should upload the contracts', function* () {
    ...
    const deployment = yield dapp.deployProject(config.dataProjectFilename, config.deployFilename);
  });
});
