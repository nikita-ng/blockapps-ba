...
describe('Product tests', function() {
  ...
  it('Fund a contract', function*() {
    ...
    const receipt = yield rest.send(admin, contract, value);
    const txResult = yield rest.transactionResult(receipt.hash);
    assert.equal(txResult[0].status, 'success');

    const balance = yield rest.getBalance(contract.address);
    //balance.should.be.bignumber.equal(value);
    assert.equal(balance.toString(), value.toString(), 'balance must be equal to the deposit');
  });
